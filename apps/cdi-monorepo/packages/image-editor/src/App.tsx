...
import { Product, PaintColor, DesignPack, Product3D, SavedDesign, ChatMessage, Product2D, PromptHistoryItem, Estimate } from './types';
...
const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        if (!storedKey) {
            ...
            setApiKey(storedKey);
            geminiService.setGeminiApiKey(storedKey);
        }
    }, []);
    ...
    const handleSaveApiKey = (key: string) => {
        ...
        setApiKey(key);
        geminiService.setGeminiApiKey(key);
    };
    ...
    const [isPromptHistoryPanelOpen, setIsPromptHistoryPanelOpen] = useState(false);
    ...
    useEffect(() => {
        if (apiKey) {
            geminiService.setGeminiApiKey(apiKey);
    ...
    // --- EXPORT HANDLER ---
    const handleExportDesign = (design: SavedDesign, destination: 'marketplace' | 'renovision') => {
        setToastMessage(`Sent "${design.name}" to ${destination === 'marketplace' ? 'Marketplace' : 'Renovision'}!`);
        // Here you could add logic to actually transfer the image to the other app/storage if needed
    };
    ...
    const handleImageDrop = useCallback(async (clientX: number, clientY: number) => {
        ...
        if (selectedProduct && sceneImageFile) {
            ...
            try {
                ...
                setPromptHistory(prev => [{ prompt: finalPrompt, imageUrl: finalImageUrl, timestamp: Date.now() }, ...prev]);
    ...
    const handleApplyTextEdit = async () => {
        if (!sceneImageFile || !textEditPrompt.trim()) return;
        ...
        try {
            ...
            const newSceneFile = dataURLtoFile(finalImageUrl, `scene-text-edited-${Date.now()}.jpg`);
            setSceneImageFile(newSceneFile);
            setTextEditPrompt('');

        } catch (e) {
            const message = e instanceof Error ? e.message : "An unknown error occurred during text edit.";
            setError(message);
        } finally {
            setIsLoading(false);
        }
    ...
    const handleSendChatMessage = async (message: string) => {
        ...
        try {
            const responseText = await geminiService.sendChatMessage(message);
            const modelMessage: ChatMessage = { role: 'model', parts: [{ text: responseText }] };
            setChatHistory(prev => [...prev, modelMessage]);
        } catch (e) {
            const message = e instanceof Error ? e.message : "An unknown error occurred with the chat.";
    ...
    const handleDownloadEstimate = (estimate: Estimate) => {
        ...
        setToastMessage("Estimate download is not yet implemented.");
    };
        ...
        <div className="w-screen h-screen bg-zinc-100 flex flex-col font-sans">
            ...
            >
                <button
                  ...
                  className="ml-4 px-3 py-1 bg-blue-100 text-blue-800 rounded text-xs font-semibold hover:bg-blue-200"
            ...
            >
                ...
                {sceneImageUrl && (
                    <>
                        {/* Show Toolbar only when no tool is active */}
                        {!activeTool && !selectedProduct && !selectedPaint && (
                            <Toolbar 
                        ...
                        {/* Show Catalog when add/paint is active or an item is selected */}
                        ...
                        {activeTool === 'erase' && (
                            <EraseControls 
                                ...
                                onApply={() => { 
                                    setToastMessage("Erase feature coming soon!");
                                    setActiveTool(null);
                                }}
                                onCancel={() => setActiveTool(null)}
                                isApplyDisabled={!hasDrawingOccurred || isLoading}
                            />
                        )}
                        ...
                        {activeTool === 'highlight' && (
                            <HighlightControls
                                ...
                                onPromptChange={setHighlightPrompt}
                                onApply={() => {
                                    setToastMessage("Highlight feature coming soon!");
                                    setActiveTool(null);
                                }}
                                onCancel={() => { setActiveTool(null); setHighlightPrompt(''); }}
                                isApplyDisabled={!hasDrawingOccurred || !highlightPrompt.trim() || isLoading}
                            />
                        )}
                        ...
                        {activeTool === 'text' && (
                            <TextEditControls
                                prompt={textEditPrompt}
                                onPromptChange={setTextEditPrompt}
                                onApply={handleApplyTextEdit}
                                onCancel={() => { setActiveTool(null); setTextEditPrompt(''); }}
                                isApplyDisabled={!textEditPrompt.trim() || isLoading}
                            />
                        )}
                    </>
                )}
                ...
                {sceneImageUrl && !activeTool && !selectedProduct && !selectedPaint && (
                    <button
                        onClick={() => setIsChatModalOpen(true)}
                        className="absolute bottom-28 right-4 z-20 bg-zinc-800 text-white rounded-full p-4 shadow-lg hover:bg-zinc-900 transition-transform hover:scale-110 animate-fade-in"
                        title="Ask Design Pro"
                    >
            ...
            <PromptHistoryPanel
                isOpen={isPromptHistoryPanelOpen}
                history={promptHistory}
                onClose={() => setIsPromptHistoryPanelOpen(false)}
            />
